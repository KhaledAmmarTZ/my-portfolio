...
export default function RightSidebar() {
  ...
  return (
    <aside className="absolute right-6 top-1/2 -translate-y-1/2 z-20 flex flex-col gap-5">

      {social.map((item) => {
        const isActive = pressed === item.id;
        ...
        return (
          <a
            ...
            className={`
              ...
              hover:border-[#D4AF37]
              hover:shadow-[0_0_18px_rgba(212,175,55,0.35)]
              active:scale-95
              ${isActive ? "shadow-[0_0_18px_rgba(212,175,55,0.45)] scale-95" : ""}
            `}
          >
            <Image
              src={isActive ? item.active : item.normal}
              alt={item.id}
              width={18}
              height={18}
            />
